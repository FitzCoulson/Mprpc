...
/*
header_size + service_name   method_name   argsize + args
*/
                              ...
                              google::protobuf::Closure *done)
{
    ...
    {
        controller->SetFailed("recive error! errno: " + std::to_string(errno));
        close(clientfd);
        return;
    }

    // std::string response_str(recv_buf,0,recv_size);//出现问题bug，recv_buf中遇到\0后面的数据就存不下来了，导致反序列化出错
